...
public class NDCGScorer extends MetricScorer {
	...
	private HashMap<Integer,Double> gainMap;
	public NDCGScorer()
	{
        this.k = 10;
	...
	public double score(RankList rl)
	{
		...
		if(rl.size() == 0) {
            System.err.println("Empty RankList!");
            return -1.0;
        }
	...
	private double getDCG(List<Integer> rel, int k)
	{
		...
		{
            if(i==1)
                dcg += gainMap.get(rel.get(i - 1));
            else
	            dcg += gainMap.get(rel.get(i - 1))/SimpleMath.logBase2(i);
		}
		return dcg;
	}
