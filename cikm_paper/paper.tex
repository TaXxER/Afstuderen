...
In recent years, several learning to rank benchmark datasets have been proposed that enable comparison of the performance of different learning to rank methods. Well-known benchmark datasets include the \emph{Yahoo! Learning to Rank Challenge} dataset \cite{Chapelle2011a}, the Yandex Internet Mathematics competition\footnote{http://imat-relpred.yandex.ru/en/}, and the LETOR dataset \cite{Qin2010} that was published by Microsoft Research.\\

No agreement exists between authors in the learning to rank field on the benchmark sets used for model evaluation. Comparing ranking performance of learning to rank methods is largely hindered by the existence of this variety of evaluation datasets and the lack of a \emph{de facto} standard way of benchmarking new models.

\section{Problem Statement \& Research Question}
\label{chap:cross_comparison}
Because the learning to rank methods evaluated differs between benchmarks, no single benchmark comparison can be regarded as a conclusive argument on which learning to rank method is most accurate.\\
