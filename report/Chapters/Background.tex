...
An algorithm to compute \ac{ERR} is shown in Listing \ref{alg:err}. The algorithm requires relevance grades $g_i$, $1 \le i \le n$ and mapping function $R$ that maps relevance grades to probability of relevance.
...
\begin{algorithm}[H]
 ...
 Output $ERR$
 \caption{Algorithm to compute the \acs{ERR} metric, obtained from \cite{Chapelle2009}}
 \label{alg:err}
\end{algorithm}
...
\subsection{Concluding notes on evaluation measures}
TODO: Ingaan op verbanden/correlaties tussen metrics.
TODO: Explain motivation for cutoff points in ranking evaluation measures.
