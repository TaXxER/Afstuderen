...
ADMM & GroupCE & RankCSA \\ 
Approx\acs{AP} & Group\acs{MLE} & RankDE \\ 
Approx\acs{nDCG} & IntervalRank & RankELM (pairwise) \\ 
BagBoo & \acs{IP}Rank & RankELM (pointwise) \\ 
Best Single Feature & KeepRank & Ranking Committee Perceptron \\ 
BL-MART & Kernel-\acs{PCA} & RankMGP \\ 
BoltzRank-Single & KL-\acs{CRF} & RankNet \\ 
BoltzRank-Pair & LAC-MR-OR & Rank\ac{RLS} \\ 
BT & LambdaMART & Rank\ac{SVM} \\ 
C-\acs{CRF} & LambdaNeuralRank & Rank\ac{SVM}-Primal \\ 
...
DCMP & ListNet & Ridge Regression \\ 
DirectRank & ListReg & RSRank \\ 
Energy\ac{nDCG} & LRUF & SmoothGrad \\ 
FBPCRank & MBP & SmoothRank \\ 
FenchelRank & MHR & SoftRank \\ 
FocusedBoost & MultiStageBoost & SortNet \\ 
FocusedNet & NewLoss & SparseRank \\ 
Focused\ac{SVM} & OWPC & \acs{SVD}-RankBoost \\ 
FPRank & PERF-\ac{MAP} & \ac{SVM}$^{\ac{MAP}}$ \\ 
FRank & PermuRank & SwarmRank \\ 
FSMRank & Q.D.\acs{KNN} & TGRank \\ 
FSM$^{\acs{MAP}}$ & RandomForest & TM \\ 
GAS-E & Rank-PMBGP & VFLR \\ 
\end{tabular}
