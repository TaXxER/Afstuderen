...
We described in section \ref{sec:ndcg} that there are two definitions of the \ac{NDCG}-metric that vary slightly: one defined by J{\"a}rvelin and Kek{\"a}l{\"a}inen \cite{Jarvelin2002} and the other defined by Burges et al. \cite{Burges2005}. The official benchmarks evaluation runs of the different benchmark sets differ in the \ac{NDCG}-definition that is used. For example, on the official LETOR evaluation runs the definition by J{\"a}rvelin and Kek{\"a}l{\"a}inen \cite{Jarvelin2002} is used, where the official evaluation runs on WCL2R use the definition by Burges et al. \cite{Burges2005}. For fair comparison, it is of importance to only take into account evaluation results that report \ac{NDCG} scores using the same \ac{NDCG}-definition as the official evaluation runs of the benchmark, therefore we filter out evaluation results that do not fulfil this property. Note that this approach results in the \ac{IWN} and \ac{NWN} calculations taking into account evaluation results in both definitions of \ac{NDCG} (although only one definition is used per data set). This is not a problem, since both definitions of \ac{NDCG} are highly correlated and vary only slightly.
