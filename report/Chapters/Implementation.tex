...
(1) TRAIN_SCA = LOAD 'train_sca[fold number]/*' USING BinStorage();
(1) TR_BY_QUERY = GROUP TRAIN_SCA BY $1 PARALLEL [number of avaiable reducers];
(1) TR_EXP_REL_SCORES = FOREACH TR_BY_QUERY GENERATE flatten(ExpRelOurScores(TRAIN_SCA));
(1) STORE TR_EXP_REL_SCORES INTO 'tr_exp_rel_scores-f[fold number]' USING BinStorage();
...
(1) VALIDATE_SCA = LOAD 'validate_sca[fold number]/*' USING BinStorage();
(1) VA_BY_QUERY = GROUP VALIDATE_SCA BY $1 PARALLEL [number of available reducers];
(1) STORE VA_BY_QUERY INTO 'va_by_query-f[fold number]' USING BinStorage();
