...
To validate out ListNet Cluster implementation we run experiments on the LETOR 3.0 OHSUMED and the MQ2007 and MQ2008 datasets from LETOR 4.0 and compare their performance in terms of \ac{nDCG}@10. Hadoop job scheduling overhead is large when processing relatively small datasets (as we will explain in more detail in section \ref{ssec:speedup}), therefore \ac{nDCG}@10 experiments are limited to five training iterations and are run only on one of the five folding combinations. Because of the stochastic nature of the ListNet ranking procedure, which is a result of random weight initialisation prior to the first training iteration, the \ac{nDCG}@10 performance is not guaranteed to be identical between multiple runs on the same input data. To account for the non-determinism of ListNet, we repeat each experiment five times and report the average \ac{nDCG}@10 as well as the standard deviation in \ac{nDCG}@10.0
\begin{table}[!h]
\centering
